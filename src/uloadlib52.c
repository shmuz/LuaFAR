...
#define LUA_PATH_DEFAULT  \
		LUA_LDIR L"?.lua;"  LUA_LDIR L"?\\init.lua;" \
		LUA_CDIR L"?.lua;"  LUA_CDIR L"?\\init.lua;" L".\\?.lua"
...
#define LUA_PATH_SEP		L";"
#endif
#if !defined (LUA_PATH_MARK)
#define LUA_PATH_MARK		L"?"
#endif
#if !defined (LUA_EXEC_DIR)
#define LUA_EXEC_DIR		L"!"
#endif
...
static void setprogdir (lua_State *L) {
  ...
  if (n == 0 || n == nsize || (lb = wcsrchr(buff, L'\\')) == NULL)
    ...
    *lb = L'\0';
    LF_Gsub(L, (const wchar_t*)lua_tostring(L, -1), LUA_EXEC_DIR, buff);
...
static const wchar_t *pushnexttemplate (lua_State *L, const wchar_t *path) {
  ...
  l = wcschr(path, *LUA_PATH_SEP);  /* find next separator */
...
static int loadfunc (lua_State *L, const wchar_t *filename, const char *modname) {
  const char *funcname;
  const char *mark;
...
static int searcher_C (lua_State *L) {
  const char *name = luaL_checkstring(L, 1);
  const wchar_t *filename = findfile(L, name, "cpath", LUA_CSUBSEP);
  if (filename == NULL) return 1;  /* module not found in this path */
